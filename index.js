...
After searching, you can use web_scrape to get full content from relevant URLs.`,
    {
      ...
      num_results: z.number().min(1).max(20).default(10).optional().describe("Number of results to return (1-20, default 10)"),
      page: z.number().min(1).max(10).default(1).optional().describe("Page number for pagination (1-10, default 1). Use if first page doesn't have what you need."),
      time_period: z.enum(["last_hour", "last_day", "last_week", "last_month", "last_year"]).optional().describe("Filter results by time period"),
      site: z.string().optional().describe("Limit search to specific site (e.g., 'github.com', 'stackoverflow.com')"),
    },
    async (args) => {
      const API_KEY = getApiKey();
      if (!API_KEY) return { content: [{ type: "text", text: "Error: SEARCHAPI_KEY not configured." }], isError: true };
      let { query, num_results, page, time_period, site } = args;
      num_results = num_results || 10;
      page = page || 1;
      try {
        let q = site ? `site:${site} ${query}` : query;
        let data = await req(API_URL, { engine: "google", q, api_key: API_KEY, page, time_period, hl: "en" });
        ...
        let body = fmtResults(results.map(r => ({ title: r.title, link: r.link, snippet: r.snippet })));
...
This is more expensive than web_search, use wisely.`,
    {
      ...
      image_url: z.string().url().optional().describe("Optional image URL for visual questions (e.g., 'What is in this image?')"),
      location: z.string().optional().describe("Location for local queries (e.g., 'New York' for 'restaurants near me')"),
    },
    async (args) => {
      const API_KEY = getApiKey();
      if (!API_KEY) return { content: [{ type: "text", text: "Error: SEARCHAPI_KEY not configured." }], isError: true };
      ...
      try {
        ...
        if (data.reference_links?.length) {
          out += "\n---\nSources:\n";
          data.reference_links.slice(0, 8).forEach(ref => { out += `[${ref.index}] ${ref.title}\n    ${ref.link}\n`; });
        }
...
The content is automatically cleaned of scripts, styles, and navigation elements.`,
    {
      ...
      selector: z.string().optional().describe("Optional CSS selector to extract specific elements (e.g., 'article', '.main-content', '#post-body')"),
      extract_mode: z.enum(["text", "markdown", "structured"]).default("text").optional().describe("Output format: 'text' (plain), 'markdown' (preserve formatting), 'structured' (headings + paragraphs)"),
      include_links: z.boolean().default(false).optional().describe("Include links found in the content"),
      max_length: z.number().min(1000).max(50000).default(10000).optional().describe("Maximum content length (1000-50000, default 10000)"),
    },
    async (args) => {
      ...
      try {
        let res = await axios.get(url, { headers: HEADERS, timeout: 20000, maxRedirects: 5, validateStatus: s => s < 400 });
        let $ = cheerio.load(res.data);
        $(JUNK_SELECTORS).remove();
        let title = $("title").text().trim() || $('meta[property="og:title"]').attr("content") || "Untitled";
        let desc = $('meta[name="description"]').attr("content") || $('meta[property="og:description"]').attr("content") || "";
        let author = $('meta[name="author"]').attr("content") || "";
        let date = $('meta[property="article:published_time"]').attr("content") || $("time[datetime]").attr("datetime") || "";
        let $main;
        if (selector) { $main = $(selector); }
        else {
          for (let sel of CONTENT_SELECTORS) {
            let el = $(sel);
            if (el.length && el.text().trim().length > 300) { $main = el; break; }
          }
        }
        ...
        if (include_links) {
          $main.find("a[href]").each((_, el) => {
            ...
            if (!txt) return;
            try { links.push({ text: txt.slice(0, 80), url: new URL(href, url).href }); } catch {}
          });
        }
...
RETURNS: List of links with their anchor text and URLs (max 50 links).`,
    {
      ...
      filter: z.string().optional().describe("Optional text filter - only return links containing this text in URL or anchor"),
    },
    async (args) => {
      let { url, filter } = args;
      try {
...
Limited to 5 URLs to avoid timeout.`,
    {
      ...
      max_per_page: z.number().min(500).max(5000).default(2000).optional().describe("Max content length per page (500-5000, default 2000)"),
    },
    async (args) => {
...
if (mode === "http") {
  (async () => {
    ...
    app.use((req, res, next) => {
      ...
      if (apiKey) req.userApiKey = apiKey;
      next();
    });
    ...
    app.post("/mcp", async (req, res) => {
      ...
      if (req.userApiKey) process.env.SEARCHAPI_KEY = req.userApiKey;

      try {
        let transport;
        if (sid && sessions[sid]) {
          transport = sessions[sid];
        } else if (!sid && isInit(req.body)) {
          transport = new StreamableHTTPServerTransport({
            sessionIdGenerator: () => randomUUID(),
            onsessioninitialized: (id) => {
              sessions[id] = transport;
              console.error(`[http] session ${id} started`);
            }
          });
          transport.onclose = () => {
            if (transport.sessionId) {
              delete sessions[transport.sessionId];
              console.error(`[http] session ${transport.sessionId} closed`);
            }
          };
          const server = new McpServer({ name: "mcp-websearch", version: "2.0.0" });
          registerTools(server);
          await server.connect(transport);
        } else {
          return res.status(400).json({ jsonrpc: "2.0", error: { code: -32000, message: "Bad Request: No valid session" }, id: null });
        }
    ...
    app.get("/mcp", async (req, res) => {
      const sid = req.headers["mcp-session-id"];
      if (!sid || !sessions[sid]) return res.status(400).send("Invalid session");
      if (req.userApiKey) process.env.SEARCHAPI_KEY = req.userApiKey;
    ...
    app.listen(port, () => console.error(`[http] MCP server on http://localhost:${port}/mcp`));

    process.on("SIGINT", async () => {
      for (const sid in sessions) await sessions[sid].close().catch(() => {});
      process.exit(0);
    });
  })();
} else {
