...
if (mode === "http") {
  // Streamable HTTP mode (MCP spec 2025-03-26)
  (async () => {
    ...
    app.post("/mcp", async (req, res) => {
      ...
      try {
        ...
        if (sid && sessions[sid]) {
          ...
          // re-register tools on new server instance
    ...
    app.listen(port, () => {
      console.error(`[http] MCP server on http://localhost:${port}/mcp`);
    });

    process.on("SIGINT", async () => {
      for (const sid in sessions) {
        await sessions[sid].close().catch(() => {});
      }
      process.exit(0);
    });
  })();
...
// tool registration function (reusable for http mode)
